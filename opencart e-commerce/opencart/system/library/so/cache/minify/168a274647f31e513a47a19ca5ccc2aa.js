...
break;}}
var getSource=function(){switch(settings.provider.toLowerCase()){case'youtube':return'//youtube.com/embed/'+settings.id+'?rel=0&autoplay=1'
+'&controls='+(settings.controls+0)
+'&showinfo='+(settings.showinfo+0);break;case'vimeo':return'//player.vimeo.com/video/'+settings.id+'?autoplay=1'
+'&color='+settings.color
+'&title='+(settings.title+0)
+'&byline='+(settings.byline+0)
+'&portrait='+(settings.controls+0);break;}}
...
$that.find('*').addBack().click(function(){setIframe();});}else{setIframe();}
return this;};$(document).ready(function(){if($('[data-easy-embed]').length>0){$('[data-easy-embed]').each(function(){$(this).easyEmbed();})}})})(jQuery,window,document);